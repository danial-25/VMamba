...
def cross_merge_fwd(
    ...
    if out_channel_first:
        ...
        if scans == 0:
            ...
            oy = oy + torch.rot90(
                y.view(B, K, D, W, H)[:, 1, :, :, :], -1, dims=(2, 3)
            ).flatten(2, 3)
            oy = oy + torch.rot90(
                y.view(B, K, D, H, W)[:, 2, :, :, :], -2, dims=(2, 3)
            ).flatten(2, 3)
            oy = oy + torch.rot90(
                y.view(B, K, D, W, H)[:, 3, :, :, :], -3, dims=(2, 3)
            ).flatten(2, 3)
            y = oy
    else:
        B, H, W, K, D = y.shape
        ...
        if scans == 0:
            ...
            oy = oy + torch.rot90(
                y.view(B, W, H, K, D)[:, :, :, 1, :], -1, dims=(1, 2)
            ).flatten(1, 2)
            oy = oy + torch.rot90(
                y.view(B, H, W, K, D)[:, :, :, 2, :], -2, dims=(1, 2)
            ).flatten(1, 2)
            oy = oy + torch.rot90(
                y.view(B, W, H, K, D)[:, :, :, 3, :], -3, dims=(1, 2)
            ).flatten(1, 2)
            y = oy
...
def cross_scan_fn(
    ...
    if x.is_cuda:
        ...
        return CrossScanF.apply(
            x, in_channel_first, out_channel_first, one_by_one, scans
        )
...
def cross_merge_fn(
    ...
    if y.is_cuda:
        ...
        return CrossMergeF.apply(
            y, in_channel_first, out_channel_first, one_by_one, scans
        )
...
class Linear(nn.Linear):
    ...
    def forward(self, x: torch.Tensor):
        if self.channel_first:
            ...
            if len(x.shape) == 4:
                return F.conv2d(
                    x, self.weight[:, :, None, None], self.bias, groups=self.groups
                )
            elif len(x.shape) == 3:
                return F.conv1d(
                    x, self.weight[:, :, None], self.bias, groups=self.groups
                )
        else:
    ...
    def _load_from_state_dict(
        ...
        if prefix + "weight" in load_state_dict_keys:
            state_dict[prefix + "weight"] = state_dict[prefix + "weight"].view_as(
                self_state_dict["weight"]
            )
...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,
        ...
        self.in_proj = Linear(
            self.d_model, d_proj, bias=bias, channel_first=channel_first
        )
        self.act: nn.Module = act_layer()
        ...
        self.out_proj = Linear(
            self.d_inner, self.d_model, bias=bias, channel_first=channel_first
        )
        self.dropout = nn.Dropout(dropout) if dropout > 0.0 else nn.Identity()
        ...
        self.dt_projs.weight.data = self.dt_projs_weight.data.view(
            self.dt_projs.weight.shape
        )
    ...
    def _load_from_state_dict(
        ...
        for k, v in names.items():
            if (prefix + k in load_state_dict_keys) and (k not in self_state_dict_keys):
                ...
                state_dict[prefix + v] = state_dict[prefix + k].view_as(
                    self_state_dict[v]
                )
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.classifier = nn.Sequential(
            OrderedDict(
                norm=LayerNorm(
                    self.num_features, channel_first=self.channel_first
                ),  # B,H,W,C
                permute=(
    ...
    def _make_downsample(
        dim=96, out_dim=192, norm=True, channel_first=False, version="v1"
    ):
        # if channel first, then Norm and Output are both channel_first
        if version == "v1":  # patch merging from swin transformer
            ...
            return nn.Sequential(
                ...
                (
                    LayerNorm(4 * dim, channel_first=channel_first)
                    if norm
                    else nn.Identity()
                ),
                Linear(
                    4 * dim,
                    (2 * dim) if out_dim < 0 else out_dim,
                    bias=False,
                    channel_first=channel_first,
                ),
            )
            ...
            return nn.Sequential(
                ...
                (
                    LayerNorm(
                        out_dim, in_channel_first=True, out_channel_first=channel_first
                    )
                    if norm
                    else (nn.Identity() if channel_first else Permute(0, 2, 3, 1))
                ),
    ...
    def _load_from_state_dict(
        ...
        for i in range(100):
            ...
            change_name(
                f"layers.{i}.downsample.reduction", f"layers.{i}.downsample.{2}"
            )
...
def get_val_loader(
    batch_size=64, root="./val", img_size=224, sequential=True, num_workers=0
):
